...
type BulkTopic = keyof typeof BULK_AI_TOPICS;
...
function isBulkTopic(value: string): value is BulkTopic {
  return BULK_AI_TOPIC_KEYS.includes(value);
}
...
export async function POST(request: Request) {
  try {
    ...
    if (isBulk) {
      ...
      const topic = asString(body.topic, "custom").trim() || "custom";
      if (!isBulkTopic(topic)) {
        return NextResponse.json(
      ...
      const found: ComposeContact[] = (contacts ?? []) as ComposeContact[];
      if (found.length === 0) {
        return NextResponse.json(
