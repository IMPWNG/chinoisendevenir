...
export async function POST(request: Request) {
  try {
    ...
    if (body.saveNotes) {
      const message = asString(body.client_message).trim();
      if (!message) {
        return NextResponse.json(
    ...
    try {
      ...
      console.warn("matching documents:", error);
      documents = [];
      adminDocuments = [];
    }
    const overrides =
    ...
    const payload = compactMatchingResult(result, overrides);
    let saved = null;
    try {
      ...
      console.warn("Matching save failed:", errorMessage(saveError));
      return NextResponse.json({
        success: true,
        saved: null,
        save_error: errorMessage(saveError),
        ...result,
      });
    }
    ...
    try {
      ...
      console.warn("matching history:", error);
      // history is optional
    }
