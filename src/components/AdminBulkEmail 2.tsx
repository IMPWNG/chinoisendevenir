...
export default function AdminBulkEmail({
  ...
  const selected = useMemo(
    () => contacts.filter((contact: BulkContact) => selectedIds.includes(contact.id)),
    [contacts, selectedIds],
  );
  const recipients = useMemo(
    () => selected.filter((contact: BulkContact) => Boolean(contact.email)),
    [selected],
  );
  ...
  const allFilteredSelected =
    ...
    filteredContacts.every((contact: BulkContact) => selectedIds.includes(contact.id));

  const toggleFiltered = () => {
    const filteredIds = filteredContacts.map((contact: BulkContact) => contact.id);
    if (allFilteredSelected) {
      onSelectedIdsChange((prev) =>
        prev.filter((id: string) => !filteredIds.includes(id)),
      );
      return;
    }
