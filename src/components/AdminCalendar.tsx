...
function contactLabel(contact?: CalendarContact | null) {
  if (!contact) return "";
  return `${contact.prenom || ""} ${contact.nom || ""}`.trim() || contact.email || "";
}

function splitHm(hm: string) {
...
export default function AdminCalendar({
  ...
  onOpenContact?: (contact: CalendarContact) => void;
}) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(errorMessage(err) === "SESSION" ? t("sessionExpired") : t("genericError"));
    } finally {
      setLoading(false);
    }
  }, [weekFrom, weekTo, t]);
  ...
  const eventsByDay = useMemo(() => {
    const map: Record<string, CalendarEvent[]> = Object.fromEntries(
      days.map((day) => [day, []]),
    );
  ...
  function openCreate(ymd: string, hour: number, minute = 0) {
    setActiveEvent(null);
    setDraft({
      ymd,
  ...
  async function saveDraft(event?: { preventDefault?: () => void }) {
    ...
    try {
      ...
      alert(errorMessage(err) === "SESSION" ? t("sessionExpired") : t("genericError"));
    } finally {
      setSaving(false);
    }
  }
  ...
  function patchDraft(updater: (prev: Draft) => Draft) {
    ...
    <div className="bg-slate-800/40 backdrop-blur-md rounded-2xl shadow-2xl p-5 mb-8 border border-slate-700/50">
      ...
      {eventsByDay.overflow.length > 0 ? (
        <div className="mb-4 flex flex-wrap gap-2">
          {eventsByDay.overflow.map((event: CalendarEvent) => (
            <button
              key={event.id}
              type="button"
              onClick={() => {
            ...
            <div className="grid grid-cols-[56px_repeat(7,minmax(0,1fr))] gap-px relative">
              ...
              {days.map((ymd) => (
                ...
                >
                  ...
                  {(eventsByDay.map[ymd] || []).map((event: CalendarEvent) => {
                    ...
                    return (
                      <button
                        ...
                        className={`absolute left-1 right-1 rounded-md border px-1.5 py-0.5 text-left shadow-lg overflow-hidden z-10 ${
                          KIND_STYLES[event.kind || "appel"] || KIND_STYLES.appel
                        }`}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            <input
              type="date"
              value={draft.ymd}
              onChange={(e) => patchDraft((prev) => ({ ...prev, ymd: e.target.value }))}
            ...
            <select
              ...
              onChange={(e) => patchDraft((prev) => ({ ...prev, timeZone: e.target.value }))}
              className="px-3 py-2 bg-slate-800 border border-slate-600 rounded-xl text-white text-sm"
            >
            ...
            <div className="flex gap-2">
              <select
                ...
                onChange={(e) =>
                  patchDraft((prev) => ({ ...prev, duration: Number(e.target.value) }))
                }
                className="flex-1 px-3 py-2 bg-slate-800 border border-slate-600 rounded-xl text-white text-sm"
              >
              ...
              <select
                value={draft.kind}
                onChange={(e) => patchDraft((prev) => ({ ...prev, kind: e.target.value }))}
                className="flex-1 px-3 py-2 bg-slate-800 border border-slate-600 rounded-xl text-white text-sm"
              >
          ...
          <div className="flex flex-wrap gap-2">
            {filteredContacts.map((c: CalendarContact) => (
              <button
                key={c.id}
                type="button"
                onClick={() =>
                  patchDraft((prev) => ({
                    ...
                    timeZone:
                      prev.timeZone === CALENDAR_TZ
                        ? String(guessTimeZoneFromCountry(c.pays) || CALENDAR_TZ)
                        : prev.timeZone,
                  }))
                }
          ...
          <div className="flex flex-wrap gap-2">
            {activeEvent.contact && onOpenContact ? (
              <button
                type="button"
                onClick={() => {
                  const opened = activeEvent.contact;
                  if (opened) onOpenContact(opened);
                }}
