...
type MatchingContact = { id: string };
...
export default function AdminMatchingPanel({
  ...
  const selected = useMemo(
    () =>
      result?.matches?.find((item: MatchItem) => item.university_id === selectedId) ||
      result?.matches?.[0],
    [result, selectedId],
  );

  const applyResult = (payload: MatchingResult | null | undefined, meta?: MatchingRun | Record<string, unknown>) => {
    if (!payload) return;
    const info = meta || {};
    setResult(payload);
    ...
    if (typeof info === "object" && info && "created_at" in info && info.created_at) {
      setSavedInfo(
        `Sauvegardé le ${new Date(String(info.created_at)).toLocaleString("fr-FR")}`,
      );
    }
  };
  ...
  const loadRuns = async ({ restore = false } = {}) => {
    try {
      ...
      console.warn("matching runs load:", error);
    }
  };
          ...
          {result.excluded?.length ? (
            <details className="text-sm text-slate-400">
              ...
              <ul className="mt-2 space-y-1">
                {result.excluded.slice(0, 20).map(
