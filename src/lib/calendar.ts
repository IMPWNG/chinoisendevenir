...
export function pad2(value: unknown) {
  return String(value).padStart(2, "0");
}

export function normalizeHm(hm: unknown) {
...
function foldCountry(value: unknown) {
  return String(value || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/\p{M}/gu, "");
...
export function timezoneLabel(timeZone: unknown, lang = "fr") {
  const row = APPOINTMENT_TIMEZONES.find((item) => item.id === timeZone);
  if (!row) return String(timeZone || CALENDAR_TZ);
  if (lang === "zh") return row.zh;
  if (lang === "en") return row.en;
  return row.fr;
}

export function guessTimeZoneFromCountry(pays: unknown) {
...
function zonedParts(date: Date, timeZone = CALENDAR_TZ) {
...
export function zonedLocalToUtc(ymd: unknown, hm: unknown, timeZone = CALENDAR_TZ) {
...
export function convertWallClock(
  ymd: unknown,
  hm: unknown,
  fromTimeZone: unknown,
  toTimeZone = CALENDAR_TZ,
  durationMinutes = DEFAULT_DURATION_MINUTES,
  locale = "fr-FR",
) {
  const utc = zonedLocalToUtc(ymd, hm, String(fromTimeZone || CALENDAR_TZ));
  const end = new Date(
    utc.getTime() +
...
export function formatYmd(date: Date, timeZone = CALENDAR_TZ) {
  const p = zonedParts(date, timeZone);
  return `${p.year}-${pad2(p.month)}-${pad2(p.day)}`;
}

export function formatHm(date: Date, timeZone = CALENDAR_TZ) {
  const p = zonedParts(date, timeZone);
  return `${pad2(p.hour)}:${pad2(p.minute)}`;
}

export function addDaysYmd(ymd: unknown, days: unknown) {
  const [year, month, day] = String(ymd).split("-").map(Number);
  const next = new Date(Date.UTC(year, month - 1, day + Number(days)));
  return `${next.getUTCFullYear()}-${pad2(next.getUTCMonth() + 1)}-${pad2(next.getUTCDate())}`;
}

export function startOfWeekYmd(ymd: unknown) {
  const [year, month, day] = String(ymd).split("-").map(Number);
...
export function weekdayIndex(ymd: unknown) {
  const [year, month, day] = String(ymd).split("-").map(Number);
  return new Date(Date.UTC(year, month - 1, day)).getUTCDay();
}

export function minutesOfDay(date: Date, timeZone = CALENDAR_TZ) {
  const p = zonedParts(date, timeZone);
...
export function parseIso(value: unknown) {
