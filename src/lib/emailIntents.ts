import { SITE_URL, wrapEmailHtml, escapeHtml } from "./emailLayout";
import { asString } from "./request";

export const AUTO_REPLY_MARKER = (intentKey: string) => `[auto:${intentKey}]`;
...
function pageCtaHtml(href: string, label: string): string {
  return `
            <div class="cta">
...
export const EMAIL_INTENTS: Record<EmailIntentKey, EmailIntent> = {
  bourses: {
    key: "bourses",
...
const INTENT_ORDER: EmailIntentKey[] = [
  "bourses",
  "visa",
  "langue",
  "tarifs",
...
export function normalizeIntentText(text: unknown): string {
  return String(text || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
export function detectEmailIntent(
  ...
  let best: EmailIntent | null = null;
  let bestScore = 0;
...
export function autoReplyMarker(intentKey: string): string {
  return AUTO_REPLY_MARKER(intentKey);
}

export function descriptionHasAutoMarker(
  description: unknown,
  intentKey: string,
): boolean {
...
export function alreadySentIntentReply(
  actions: unknown,
  intentKey: string,
): boolean {
  const rows = (Array.isArray(actions) ? actions : []) as SuiviActionRow[];
  if (intentKey === "tarifs") {
...
export function shouldSkipIntentAutoReply(
...
export function splitPersonName(fullName: unknown): { prenom: string; nom: string } {
...
export function displayNameFromFrom(from: unknown): string {
  if (!from) return "";
  if (Array.isArray(from)) return displayNameFromFrom(from[0]);
  if (typeof from === "object") {
    const row = from as { name?: unknown; displayName?: unknown };
    return asString(row.name || row.displayName).trim();
  }
...
export function nameFromBody(text: unknown): string {
...
export function shouldCreateContactFromInbound(
  text: unknown,
  subject: unknown = "",
): boolean {
...
export function generateBoursesReplyTemplate(prenom: unknown): string {
  return wrapEmailHtml({
    title: "Bourses d'études en Chine",
    subtitle: "Une aide possible, jamais automatique",
    prenom: asString(prenom),
    bodyHtml: `
            <div class="section">
...
export function generateVisaReplyTemplate(prenom: unknown): string {
  return wrapEmailHtml({
...
export function generateLangueReplyTemplate(prenom: unknown): string {
  return wrapEmailHtml({
...
export function generateAdmissionReplyTemplate(prenom: unknown): string {
  return wrapEmailHtml({
...
export function generateProcessusReplyTemplate(prenom: unknown): string {
  return wrapEmailHtml({
...
export function generateGeneralReplyTemplate(prenom: unknown): string {
  return wrapEmailHtml({
