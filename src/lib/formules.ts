...
export type FormuleIncludeGroup = { title: string; items: string[] };

export type FormuleWhyChoose = {
  title: string;
  intro: string;
  steps: string[];
};
...
export type Formule = {
  ...
  idealIf: string[];
  cta: string;
  includeGroups?: FormuleIncludeGroup[];
  savings?: string;
  savingsLabel?: string;
  savingsText?: string;
  whyChoose?: FormuleWhyChoose;
};

export type FormuleAccess = {
  number: number;
  matchLimit: number;
  depth: string;
  documents: boolean;
...
type FormuleLoc = {
  ...
  savingsText?: string;
  whyChoose?: FormuleWhyChoose;
  groups?: FormuleIncludeGroup[];
};

const FORMULE_3_GROUPS: FormuleIncludeGroup[] = [
  {
...
export function getFormuleIncludeGroups(formule: Formule | null | undefined) {
...
export function displayFormuleLabel(formuleLabel: unknown) {
  ...
  return formuleLabel ? String(formuleLabel) : "Formule sélectionnée";
}

export function getFormuleByNumber(number: unknown) {
  return FORMULES.find((item) => item.number === number) || null;
}

export function canonicalFormuleValue(formuleLabel: unknown) {
  const found = getFormuleByNumber(getFormuleNumber(formuleLabel));
  if (found) return found.value;
  return formuleLabel ? String(formuleLabel).trim() : "";
}

export function getFormuleAccess(number: unknown): FormuleAccess {
...
export function getUnlockedStudentAccess(formuleNumber: unknown) {
  if (!formuleNumber) return getFormuleAccess(0);
  return getFormuleAccess(formuleNumber);
}

export function displayFormuleFootnote(footnote: unknown) {
  if (!footnote) return "";
...
export function localizeFormule(formule: Formule, dict?: SiteCopy | null): Formule {
  ...
  const next: Formule = {
    ...formule,
    title: loc.title ?? formule.title,
    subtitle: loc.subtitle ?? formule.subtitle,
    shortTitle: loc.title ?? formule.shortTitle,
    audience: loc.audience ?? formule.audience,
    intro: loc.intro ?? formule.intro,
    footnote: loc.footnote ?? formule.footnote,
    cta: loc.cta ?? formule.cta,
