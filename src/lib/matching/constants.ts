...
export const CATEGORY_META: Record<
  string,
  { key: string; label: string; subtitle: string; clientLabel: string }
> = {
  safety: {
    key: "safety",
    label: "Sûre",
...
export function diplomaToTargetDegree(diplome: unknown) {
...
export function categoryKeyFromScore(score: unknown) {
  ...
  if (n >= 80) return "safety";
  if (n >= 60) return "match";
  if (n >= 40) return "reach";
  return "unready";
}

export function categoryFromScore(score: unknown) {
  return CATEGORY_META[categoryKeyFromScore(score)].label;
}

export function categoryMetaFromScore(score: unknown) {
  return CATEGORY_META[categoryKeyFromScore(score)];
}

export function priorityFromScore(score: unknown) {
  const n = Number(score) || 0;
  if (n >= 80) return "Haute";
  if (n >= 60) return "Moyenne";
  if (n >= 40) return "Moyenne-basse";
  return "Basse";
}

export function englishToIelts(english: unknown) {
...
export function englishToToefl(english: unknown) {
  const value = normalizeText(english);
  if (!value) return null;
  if (value === "none" || value === "aucun") return 0;
...
export function infoStatus(kind: unknown, text: unknown) {
  return { kind, text };
}
