...
import type { Mixable } from "./mix";
import type { MatchingUniversity } from "./university";

export type MatchingGap = {
...
function monthsForHskGap(gap: number) {
  if (gap <= 0) return 0;
  if (gap === 1) return 4;
  if (gap === 2) return 8;
  return 12;
...
export function identifyGaps(
  ...
  universities: Mixable[] = [],
) {
  const gaps: MatchingGap[] = [];
  const seen = new Set();

  (universities || []).forEach((item) => {
    const uni = (item.university || {}) as MatchingUniversity & {
      name?: string | null;
    };
...
export function summarizeGaps(gaps: MatchingGap[] | null | undefined) {
  const byType: Record<string, number> = { langue: 0, academique: 0, financier: 0, autre: 0 };
  (gaps || []).forEach((gap) => {
