...
type ScoreBreakdown = Record<
  string,
  { points?: number; max?: number; note?: string; status?: string }
>;
...
export type UniversityMatch = {
  ...
  strengths?: string[];
  warnings?: string[];
  confirmed_information?: unknown;
  estimated_information?: unknown;
  ...
  cost_estimate?: {
    ...
    tuition_cny?: number | string | null;
    tuition_cny_max?: number | string | null;
    status?: string;
  };
  ...
  recommended_formula?: number | null;
  pedagogical?: boolean;
  excludeReason?: string | null;
  breakdown?: ScoreBreakdown;
};

function money(cny: unknown) {
  if (cny == null) return "à confirmer";
  return `${Number(cny).toLocaleString("fr-FR")} RMB / an`;
}

function formulaLabel(number: unknown) {
  const f = getFormuleByNumber(number);
  if (!f) return "à confirmer selon le projet";
  return `Formule ${f.number} — ${f.shortTitle} (${displayFormulePrice(f)})`;
}

function depthForFormula(number: unknown) {
  if (number === 3) return "complete";
  if (number === 2) return "candidature";
  return "orientation";
}

function qualitativeFromBreakdown(match: UniversityMatch) {
...
export function buildUniversityAnalysis(match: UniversityMatch, student: MatchingStudent) {
  ...
  const meta = CATEGORY_META[match.categoryKey || "match"] || CATEGORY_META.match;
  const uniName = match.university_name || match.name || "Université";
  const summary = `${uniName} obtient ${match.score}/100 (${meta.label}). ${qualitativeFromBreakdown(match)}`;
...
export function buildClientMessage(
  ...
  analyses: UniversityMatch[],
  overallFormula: unknown,
  { gaps = [] }: { gaps?: MatchingGap[] } = {},
) {
  ...
  if (!top.length) {
    ...
    top.forEach((item, index: number) => {
      const detail =
        depth === "orientation"
          ? item.qualitative
          : `${item.score}/100`;
      lines.push(
        `${index + 1}. ${item.university_name || item.name} — ${item.category}. ${detail}.`,
      );
  ...
  const docs = [
    ...new Set(
      top.flatMap((item) =>
        (item.missing_documents || []).filter(
          (doc): doc is string | { key?: string; status?: string; label?: string } =>
            doc != null,
        ),
      ),
    ),
  ...
  if (
    ...
    docs.forEach((doc) =>
      lines.push(`• ${typeof doc === "string" ? doc : doc.label || doc.key || "document"}`),
    );
  }
...
export function buildInternalBrief(
  student: MatchingStudent,
  analyses: UniversityMatch[],
  excluded: Array<{ university_name?: string | null; excludeReason?: string | null }>,
  overallFormula: unknown,
  extra: { mixCounts?: unknown; gaps?: MatchingGap[] } = {},
) {
  ...
  return {
    ...
    top_match: top
      ? { name: top.university_name || top.name, score: top.score, category: top.category }
      : null,
