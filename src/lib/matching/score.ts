...
function diplomaFitsTarget(student: MatchingStudent) {
...
function intakeTooFar(student: MatchingStudent, university: MatchingUniversity) {
  ...
  if (months.includes(student.intake.month)) return false;
    ...
    (best: number, month: number) =>
      Math.min(best, Math.abs(month - (student.intake.month ?? month))),
    12,
  );
  return closest >= 5;
}

function hardFilter(student: MatchingStudent, university: MatchingUniversity) {
...
function scoreLangue(
  ...
  if (studentHsk != null && hskRequired != null) {
    ...
    score += (studentIelts ?? 0) >= 6 ? 20 : (studentIelts ?? 0) >= 5 ? 10 : 0;
  }
...
function scoreAcademique(
  student: MatchingStudent,
  university: MatchingUniversity,
  flags: ScoreFlags,
  overlap: DomainSimilarity,
) {
  let score = 50;
...
export function matchUniversity(
  student: MatchingStudent,
  university: MatchingUniversity,
  { pedagogical = false }: { pedagogical?: boolean } = {},
) {
...
export function rankMatches(
  student: MatchingStudent,
  universities: MatchingUniversity[],
) {
  const pedagogical = student.formuleNumber === 1;
  const ranked: ReturnType<typeof matchUniversity>[] = [];
  const excluded: ReturnType<typeof matchUniversity>[] = [];
