...
function absoluteUrl(path: unknown) {
  ...
  return `${SITE.url}${pathText.startsWith("/") ? pathText : `/${pathText}`}`;
}
...
export function organizationJsonLd() {
  return {
    ...
    knowsAbout: [
      "Étudier en Chine",
      "Admission dans une université chinoise",
...
export function serviceJsonLd() {
  return {
    "@context": "https://schema.org",
    "@type": "Service",
    name: "Accompagnement pour étudier en Chine",
    serviceType: "Orientation, admission, bourses et visa étudiant",
    provider: {
      "@type": "Organization",
      name: SITE.name,
      url: SITE.url,
