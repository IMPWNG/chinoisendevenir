...
export type ContactRow = {
  ...
  dossier_etape?: string | number | null;
...
export function getStudentStepIndex(statut: unknown) {
  if (!statut) return 0;
  const key = canonicalStatut(statut) || String(statut);
  return STATUS_STEP_INDEX[key] ?? 0;
}

export function clampDossierEtape(value: unknown) {
...
export function getDisplayedStepIndex(contact: ContactRow | null | undefined) {
...
export function mergeAvancementNote(notesAdmin: unknown, etapeIndex: unknown) {
  const noteLine = `Avancement dossier: ${etapeIndex}`;
  const cleaned = stripAvancementNote(notesAdmin);
  return cleaned ? `${cleaned}\n${noteLine}` : noteLine;
}

export function stripAvancementNote(notesAdmin: unknown) {
...
export function hasFilledLeadForm(contact: ContactRow | null | undefined) {
  if (!contact) return false;
  const required = [
    contact.prenom,
    contact.nom,
...
export function getPaidFormuleNumber(contact: ContactRow | null | undefined) {
  if (!isFormulePaid(contact)) return null;
  return getFormuleNumber(getChosenFormule(contact)) || 1;
}

export function studentCanAccessDocuments(contact: ContactRow | null | undefined) {
  if (!isStudentAccessGranted(contact)) return false;
  return Number(getGrantedFormuleNumber(contact)) >= 2;
}

export function getVisibleStudentSteps(formuleNumber: unknown) {
...
export function diplomaLevelFromStudent(dernierDiplome: unknown) {
...
export function getRequiredStudentDocuments(
  student: { dernier_diplome?: unknown } = {},
) {
...
export function legacyDiplomaDocKey(dernierDiplome: unknown) {
  const level = diplomaLevelFromStudent(dernierDiplome);
...
export function studentHasDiplomaUpload(
  documents: Array<string | StudentDocRef | null | undefined> = [],
) {
  const keys = new Set(
    (documents || [])
...
export function studentCanAccessVisaDocuments(formuleNumber: unknown) {
  const n = Number(formuleNumber);
  return n === 1 || n >= 3;
}

export function getChosenFormule(contact: ContactRow | null | undefined) {
  if (!contact) return "";
...
export function mergeFormuleNote(notesAdmin: unknown, formuleLabel: unknown) {
  const noteLine = `Formule choisie: ${formuleLabel}`;
  const cleaned = stripFormuleNote(notesAdmin);
  return cleaned ? `${cleaned}\n${noteLine}` : noteLine;
}

export function stripFormuleNote(notesAdmin: unknown) {
  return String(notesAdmin || "")
