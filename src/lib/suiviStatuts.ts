...
export const LEGACY_STATUT_MAP: Record<string, string> = {
...
export function canonicalStatut(value: unknown) {
...
export function toStoredStatut(value: unknown) {
  const canonical = canonicalStatut(value);
  if (!canonical) return canonical;
  return CANONICAL_TO_STORED_STATUT[canonical] || canonical;
}

export function shouldAdvanceStatus(currentStatus: unknown, nextStatus: unknown) {
...
export function isFormuleAlreadyChosen(statut: unknown) {
  return FORMULE_ALREADY_CHOSEN.has(canonicalStatut(statut));
}

export function isFormulesAwaitingReply(statut: unknown) {
  return FORMULES_AWAITING_REPLY.has(String(statut || "")) ||
