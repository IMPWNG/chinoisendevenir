...
type AdmissionExtra = {
  has_csc?: boolean;
  english_programs_available?: boolean;
  chinese_language_program_available?: boolean;
  presentation?: string;
  requirements?: Record<string, AdmissionRequirement | null | undefined>;
  programs?: Array<{ level?: string; name?: string; language?: string }>;
  scholarships?: Array<{
    name?: string;
    coverage?: string;
  ...
  application?: {
    ...
    intake_months?: Array<number | string>;
    application_fee_cny?: number | string | null;
  };
  language?: {
    hsk_bachelor?: number | string | null;
    ielts_min?: number | string | null;
    toefl_min?: number | string | null;
  };
  age_max?: {
...
type UniversityRow = {
  ...
  scholarship_min?: number | string | null;
  scholarship_max?: number | string | null;
  min_hsk_level?: number | string | null;
  language_requirements?: string | null;
  tuition_min?: number | string | null;
  tuition_max?: number | string | null;
  application_deadline?: string | null;
  extra?: { admission?: AdmissionExtra } | null;
};
...
function universityToForm(row: UniversityRow): UniversityForm {
  return {
    ...EMPTY_FORM,
    ...row,
...
function formToPayload(form: UniversityForm) {
...
function replyLabel(status: unknown, t: AdminI18nValue["t"]) {
...
export default function AdminUniversities() {
  ...
  const [form, setForm] = useState<UniversityForm>(EMPTY_FORM);
  ...
  const provinces = useMemo(
    () =>
      [...new Set(universities.map((u) => u.province).filter((value): value is string => Boolean(value)))].sort(),
    [universities],
  );

  const cities = useMemo(
    () => [...new Set(universities.map((u) => u.city).filter((value): value is string => Boolean(value)))].sort(),
    [universities],
  );
  ...
  const saveUniversity = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  const deleteUniversity = async (row: UniversityRow) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-4">
                            {replyText ? (
                              <span
                                className={`text-xs px-2 py-1 rounded-lg border font-bold ${REPLY_STYLES[(u.reply_status as ReplyStatus) || "pending"]}`}
                              >
                                {replyText}
                              </span>
                            ) : (
...
function chipClass(tone: ChipTone = "slate") {
...
function AdmissionChips({ university }: { university: UniversityRow }) {
...
function UniversityModal({
  ...
  form: UniversityForm;
  setForm: Dispatch<SetStateAction<UniversityForm>>;
  saving: boolean;
  isNew: boolean;
  onClose: () => void;
  ...
  const { t } = useAdminI18n();
  const update = (name: keyof UniversityForm, value: UniversityForm[keyof UniversityForm]) =>
