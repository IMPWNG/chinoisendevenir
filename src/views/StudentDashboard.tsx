...
import type { SiteI18nValue } from "../context/SiteI18nContext";
...
type StudentMatchingData = ComponentProps<typeof StudentMatching>["matching"];
type ChineseMatchingData = ComponentProps<
  typeof StudentChineseMatching
>["chineseMatching"];

/** Form-facing fields: strings (no null) so controlled inputs type-check. */
type StudentProfile = Omit<
  ContactRow,
  | "prenom"
  | "nom"
  | "age"
...
type MeResponse = {
  ...
  matching?: StudentMatchingData;
  chineseMatching?: ChineseMatchingData;
  requiredDocuments?: StudentDoc[];
...
function translatedOption(t: SiteI18nValue["t"], prefix: string, value: string) {
...
function budgetLabel(t: SiteI18nValue["t"], value: string) {
  const mapped = BUDGET_LABEL_KEYS[value as keyof typeof BUDGET_LABEL_KEYS];
  return translatedOption(t, "form.budgets", mapped || value);
}

function catalogDoc(dict: SiteCopy, key: string) {
  const catalog = dict.student.docsCatalog;
  if (key in catalog) return catalog[key as keyof typeof catalog];
  return undefined;
}
...
export default function StudentDashboard() {
  ...
  const [matching, setMatching] = useState<StudentMatchingData>(null);
  const [chineseMatching, setChineseMatching] =
    useState<ChineseMatchingData>(null);
  const [requiredDocuments, setRequiredDocuments] = useState<StudentDoc[]>([]);
  ...
  const loadProfile = async ({ silent = false } = {}) => {
    ...
    setError("");
    try {
      const data = await studentFetch<MeResponse>("/api/student/me");
  ...
  const handleSave = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    try {
      const data = await studentFetch<{ profile: StudentProfile }>("/api/student/profile", {
        method: "PATCH",
  ...
  const handleUpload = async (e: FormEvent<HTMLFormElement>, docKey: string) => {
    e.preventDefault();
  ...
  const handleDownload = async (path: string) => {
    try {
      const data = await studentFetch<{ url?: string }>(
            ...
            <div className="student-card student-card-wide">
              ...
              <LeadForm
                ...
                initialValues={{
                  ...
                  domaine_etudes: (() => {
                    const domaine = String(profile?.domaine_etudes || "");
                    if (!domaine) return "";
                    return DOMAINES_ETUDES.includes(domaine) ? domaine : "Autre";
                  })(),
                  domaine_etudes_precision: (() => {
                    const domaine = String(profile?.domaine_etudes || "");
                    if (!domaine || DOMAINES_ETUDES.includes(domaine)) return "";
                    return domaine;
                  })(),
                    ...
                    >
                      ...
                      {diplomaOptions.map((value) => (
                        <option key={value} value={value}>
                          { (DIPLOMA_VALUES as readonly string[]).includes(value)
                                  ...
                                  >
                                    <div className="doc-row-main">
                                      <div className="doc-row-title">
                                        <span>{doc.icon}</span>
                                        {catalogDoc(dict, doc.key)?.label || doc.label}
                                        <span
                                          className={
                                      ...
                                      <p className="doc-row-desc">
                                        {catalogDoc(dict, doc.key)?.description ||
                                          doc.description}
                                      </p>
                                      {doc.file ? (
